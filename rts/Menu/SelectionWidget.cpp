...
static bool ShouldScanLuaAIScriptsInMenu()
{
...
void SelectionWidget::AddAIScriptsFromArchive()
{
	...
	// Vulkan menu startup still traverses an unstable VFS remap frontier here.
	// Keep script discovery on native AIs until the non-OpenGL path is fully migrated.
	if (!ShouldScanLuaAIScriptsInMenu())
		return;
