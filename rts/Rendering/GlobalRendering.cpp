...
CGlobalRendering::CGlobalRendering()
	: timeOffset(0.0f), lastTimeOffset(0.0f), lastFrameTime(0.0f), lastFrameStart(spring_notime), lastSwapBuffersEnd(spring_notime), weightedSpeedFactor(0.0f), drawFrame(1), FPS(1.0f)

	  ,
	  numDisplays(1)

	  ,
	  screenSizeX(1), screenSizeY(1)

	  // window geometry
	  ,
	  winPosX(configHandler->GetInt("WindowPosX")), winPosY(configHandler->GetInt("WindowPosY")), winSizeX(1), winSizeY(1)

	  // viewport geometry
	  ,
	  viewPosX(0), viewPosY(0), viewSizeX(1), viewSizeY(1), viewWindowOffsetY(0)

	  // dual viewport geometry (DualScreenMode = 1)
	  ,
	  dualWindowOffsetY(0), dualViewPosX(0), dualViewPosY(0), dualViewSizeX(0), dualViewSizeY(0)

	  ,
	  winBorder{0}

	  ,
	  winChgFrame(0), gmeChgFrame(0)

	  ,
	  screenViewMatrix(), screenProjMatrix()
	  ...
	  // pixel geometry
	  ,
	  pixelX(0.01f), pixelY(0.01f)

	  // sane defaults
	  ,
	  minViewRange(MIN_ZNEAR_DIST * 8.0f), maxViewRange(MAX_VIEW_RANGE * 0.5f), aspectRatio(1.0f)

	  ,
	  forceDisableGL4(configHandler->GetInt("ForceDisableGL4")), forceCoreContext(configHandler->GetInt("ForceCoreContext")), forceSwapBuffers(configHandler->GetInt("ForceSwapBuffers"))

	  // fallback
	  ,
	  msaaLevel(configHandler->GetInt("MSAALevel")), minSampleShadingRate(configHandler->GetFloat("MinSampleShadingRate")), maxTextureSize(2048), maxTexSlots(2), maxFragShSlots(8), maxCombShSlots(8), maxTexAnisoLvl(0.0f)

	  ,
	  drawSky(true), drawWater(true), drawGround(true), drawMapMarks(true), drawFog(true)

	  ,
	  drawDebug(false), drawDebugTraceRay(false), drawDebugCubeMap(false)

	  ,
	  glDebug(configHandler->GetBool("DebugGL")), glDebugErrors(false)

	  ,
	  teamNanospray(configHandler->GetBool("TeamNanoSpray")), active(true), compressTextures(false)

	  ,
	  haveAMD(false), haveMesa(false), haveIntel(false), haveNvidia(false), amdHacks(false)

	  ,
	  supportPersistentMapping(false), supportExplicitAttribLoc(false), supportTextureQueryLOD(false), supportMSAAFrameBuffer(false), supportDepthBufferBitDepth(16), supportRestartPrimitive(false), supportClipSpaceControl(false), supportSeamlessCubeMaps(false), supportFragDepthLayout(false), haveGL4(false)

	  ,
	  glslMaxVaryings(0), glslMaxAttributes(0), glslMaxDrawBuffers(0), glslMaxRecommendedIndices(0), glslMaxRecommendedVertices(0), glslMaxUniformBufferBindings(0), glslMaxUniformBufferSize(0), glslMaxStorageBufferBindings(0), glslMaxStorageBufferSize(0)

	  ,
	  dualScreenMode(false), dualScreenMiniMapOnLeft(false), fullScreen(configHandler->GetBool("Fullscreen")), borderless(configHandler->GetBool("WindowBorderless")), underExternalDebug(false), forceDWMFlush(configHandler->GetInt("DWMFlush")), sdlWindow{nullptr}, glContext{nullptr}, glExtensions{}, glTimerQueries{0}
{
#ifdef _WIN32
	...
	configHandler->NotifyOnChange(this, {"DualScreenMode",
...
bool CGlobalRendering::CreateWindowAndContext(const char *title)
{
	...
	const int2 minCtx = (mesaGL != nullptr && std::strlen(mesaGL) >= 3) ? int2{std::max(mesaGL[0] - '0', 3), std::max(mesaGL[2] - '0', 0)} : int2{configHandler->GetInt("GLContextMajorVersion"), configHandler->GetInt("GLContextMinorVersion")};
	...
	const GLvoid *userParam)
{
	...
	break; // "Buffer detailed info: Buffer object 260 (bound to GL_PIXEL_UNPACK_BUFFER_ARB, usage hint is GL_STREAM_DRAW) has been mapped in DMA CACHED memory."
	default:
	{
	}
	break;
