...
namespace cpu_topology {
    ProcessorCaches GetProcessorCache() { return ProcessorCaches(); }
    ProcessorMasks GetProcessorMasks() { return ProcessorMasks(); }
    ThreadPinPolicy GetThreadPinPolicy() { return THREAD_PIN_POLICY_ANY_PERF_CORE; }
}
#else
// --- ORIGINAL LINUX CODE STARTS HERE ---
#include "System/Platform/CpuTopology.h"
...
#define MAX_CPUS 32 // Maximum logical CPUs
	...
	{
		auto foundCache = std::ranges::find_if(processorCaches.groupCaches, [cacheSize](const auto &gc) -> bool
											   { return (gc.cacheSizes[2] == cacheSize); });
	...
	ProcessorGroupCaches &get_arm_group_cache(ProcessorCaches &processorCaches, uint32_t cacheSize, int cacheLevel, bool useExistingGroup)
	{
		...
		if (useExistingGroup)
		{
			auto foundCache = std::ranges::find_if(processorCaches.groupCaches, [cacheSize, cacheSizeIndex](const auto &gc) -> bool
	...
	ProcessorCaches GetProcessorCache()
	{
		...
		std::ranges::stable_sort(processorCaches.groupCaches, [cacheSizeIndex](const auto &lh, const auto &rh) -> bool
