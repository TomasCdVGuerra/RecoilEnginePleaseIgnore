...
template <uint32_t NumBuckets, size_t BucketSize>
...
template <size_t S, size_t Alignment>
struct DynMemPool
{
public:
	void *allocMem(size_t size)
...
template <size_t S, size_t N, size_t K, size_t Alignment>
struct FixedDynMemPool
{
public:
	template <typename T, typename... A>
	T *alloc(A &&...a)
...
// fixed-size version.
template <size_t N, size_t S, size_t Alignment>
...
inline size_t StablePosAllocator<T>::Allocate(size_t numElems)
{
	...
	if (positionToSize.empty())
	{
		...
		const uintptr_t threadIdForLog = reinterpret_cast<uintptr_t>(Threading::GetCurrentThreadId());
#else
		const uintptr_t threadIdForLog = static_cast<uintptr_t>(Threading::GetCurrentThreadId());
#endif
		myLog("StablePosAllocator<T>::Allocate(%u) = %u [thread_id = %" PRIuPTR "]", uint32_t(numElems), uint32_t(returnPos), threadIdForLog);
		return returnPos;
	}
