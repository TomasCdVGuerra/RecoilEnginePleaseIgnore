...
namespace agui
{
	...
	void Gui::Draw()
	{
		...
		static bool loggedGuiDrawOnce = false;
		if (!loggedGuiDrawOnce)
		{
			const int backendType = ((globalRendering != nullptr) && (globalRendering->graphicsBackend != nullptr))
										? static_cast<int>(globalRendering->graphicsBackend->Type())
										: -1;

			LOG("[Vulkan][Frontier12][Gui::Draw] first draw, elements=%zu backendType=%d", elements.size(), backendType);
			loggedGuiDrawOnce = true;
		}
