...
namespace agui
{
	...
	LineEdit::LineEdit(GuiElement *parent)
		: GuiElement(parent), hasFocus(false), crypt(false), cursorPos(0)
	{
	}
	...
	void LineEdit::DrawSelf()
	{
		...
		if (hasFocus)
		{
			...
			{
				GuiElement caret;
				caret.SetPos(csx, textCenter - cursorHeight / 2);
				caret.SetSize(cw, cursorHeight);
				caret.DrawBox(GL_QUADS, {f, f, f, opacity});
			}
		}
